...
class ImageOptimizer {
    ...
    public function processUpload($file) {
        ...
        if ($imageInfo === false) {
            return ['success' => false, 'message' => 'File is not a valid image or is corrupted'];
        }
        
        // Get actual image type from file content (more reliable than MIME type)
        $actualType = $imageInfo['mime'];
        $allowedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'];
        
        if (!in_array($actualType, $allowedTypes)) {
            return ['success' => false, 'message' => 'Invalid image type: ' . $actualType];
        }
        ...
        // Load original image using actual detected type
        $sourceImage = $this->loadImage($file['tmp_name'], $actualType);
        if (!$sourceImage) {
            return ['success' => false, 'message' => 'Failed to load image. The file may be corrupted or in an unsupported format.'];
        }
